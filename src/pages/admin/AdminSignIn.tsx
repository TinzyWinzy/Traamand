...
export default function AdminSignIn() {
  ...
  useEffect(() => {
    const role = sessionStorage.getItem('traamand_admin_expected_role')
    if (role !== 'admin') {
  ...
  return (
    <section className="bg-zinc-50 py-12 sm:py-20">
      <div className="mx-auto max-w-md px-4 sm:px-6">
        <Link to="/sign-in" className="mb-6 inline-flex items-center gap-1 text-sm text-teal-600 hover:underline">
          <ArrowLeft className="h-4 w-4" /> Back to Client Login
        </Link>

        <div className="rounded-2xl bg-white p-6 shadow-md sm:p-8">
          <div className="mb-6 text-center">
            <Shield className="mx-auto h-10 w-10 text-slate-900" />
            <h1 className="mt-3 text-2xl font-extrabold text-slate-900">Admin Access</h1>
