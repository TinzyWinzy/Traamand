...
export default function RoleSignIn({ expectedRole, successPath, title, description, showBack = true }: RoleSignInProps) {
  ...
  const isAuthorized = (role: string) => {
    if (expectedRole === 'admin') return role === 'admin' || role === 'superadmin'
  ...
  return (
    <section className="bg-zinc-50 py-12 sm:py-20">
      <div className="mx-auto max-w-md px-4 sm:px-6">
        {showBack && (
          <Link to="/" className="mb-6 inline-flex items-center gap-1 text-sm font-semibold text-teal-600 hover:text-teal-700">
            ← Back to Home
          </Link>
        )}
          ...
          <div className="p-8 sm:p-10">
            ...
            <div className="mb-6 flex rounded-xl border border-slate-200 bg-slate-50 p-1">
              <button
                ...
                className={`flex-1 rounded-lg px-3 py-2.5 text-sm font-semibold transition ${
                  mode === 'google' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'
                }`}
              >
                Google
              </button>
            ...
            {mode === 'google' ? (
              <button
                ...
                disabled={loading}
                className="flex w-full items-center justify-center gap-3 rounded-xl border border-slate-200 bg-white px-6 py-3.5 text-sm font-bold text-slate-700 shadow-sm transition hover:bg-slate-50 active:scale-[0.98] disabled:opacity-50"
              >
                {loading ? (
