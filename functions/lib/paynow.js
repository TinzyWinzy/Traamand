...
exports.processPayout = exports.payoutCallback = exports.paynowCallback = exports.pollPaynowPayment = exports.processPaynowPayment = void 0;
...
exports.processPaynowPayment = (0, https_1.onCall)({ secrets: [paynowId, paynowKey] }, async (request) => {
    ...
    const paynowConfig = {
        ...
        resultUrl: `${(0, helpers_1.getFunctionBaseUrl)()}/paynowCallback`,
        returnUrl: `${(0, helpers_1.getSiteUrl)()}/book/${booking.workerSlug || booking.workerId}/confirmation?bookingId=${bookingId}`,
    };
    ...
    const paynowData = (0, helpers_1.buildPaynowData)({
    ...
    try {
        ...
        const hashOk = (0, helpers_1.verifyPaynowHash)(params, paynowConfig.integrationKey);
        if (!hashOk) {
        ...
        const pollUrl = (0, helpers_1.getPaynowParam)(params, 'PollUrl');
        const browserUrl = (0, helpers_1.getPaynowParam)(params, 'BrowserUrl');
        if (pollUrl && browserUrl) {
            ...
            await bookingRef.update({
                ...
                platformFeePercent: helpers_1.PLATFORM_FEE_PERCENT,
                platformCutAmount: Math.round(fee * helpers_1.PLATFORM_FEE_PERCENT * 100) / 100,
                traamandNetRevenue: Math.round(fee * (1 - helpers_1.PLATFORM_FEE_PERCENT) * 100) / 100,
        ...
        const error = (0, helpers_1.getPaynowParam)(params, 'Error');
        await bookingRef.update({
...
exports.pollPaynowPayment = (0, https_1.onCall)(async (request) => {
    ...
    try {
        ...
        const text = await response.text();
        const params = new URLSearchParams(text);
        const status = (0, helpers_1.getPaynowParam)(params, 'status') || 'unknown';
        ...
        if (paid) {
            await (0, helpers_1.writePlacementFeeTransaction)(bookingId, booking);
        }
        return { paid, status };
    }
    catch (err) {
...
exports.paynowCallback = (0, https_1.onRequest)({ secrets: [paynowKey] }, async (req, res) => {
    ...
    const reference = (0, helpers_1.getPaynowParam)(params, 'reference');
    const status = (0, helpers_1.getPaynowParam)(params, 'status') || 'unknown';
    ...
    const hash = (0, helpers_1.getPaynowParam)(params, 'hash');
    if (integrationKey && hash) {
        if (!(0, helpers_1.verifyPaynowHash)(params, integrationKey)) {
            const allParams = {};
    ...
    if (paid) {
        await (0, helpers_1.writePlacementFeeTransaction)(bookingDoc.id, booking);
    }
...
exports.processPayout = (0, https_1.onCall)({ secrets: [paynowId, paynowKey] }, async (request) => {
    ...
    const fee = Math.round(amount * helpers_1.PAYOUT_FEE_PERCENT * 100) / 100;
    ...
    const returnUrl = (0, helpers_1.getFunctionBaseUrl)() + '/paynowCallback';
    const resultUrl = (0, helpers_1.getFunctionBaseUrl)() + '/payoutCallback';
    ...
    try {
        ...
        const paynowStatus = (0, helpers_1.getPaynowParam)(result, 'status');
        if (paynowStatus.toLowerCase() === 'ok') {
            const paynowReference = (0, helpers_1.getPaynowParam)(result, 'reference') || reference;
            await payoutRef.update({
                status: 'completed',
        ...
        const error = (0, helpers_1.getPaynowParam)(result, 'error') || text.slice(0, 300);
        await payoutRef.update({
            status: 'pending',
