...
export function getCommission(event: string, bookingAmount?: number): { amount: number; description: string } {
...
const CASHBACK_MILESTONES = [1, 2, 5, 10, 20] as const

export function isReferralMilestoneReached(totalReferrals: number): boolean {
  return (CASHBACK_MILESTONES as readonly number[]).includes(totalReferrals)
}
...
export function getMilestoneForCount(totalReferrals: number): number | null {
  if (CASHBACK_MILESTONES.includes(totalReferrals as typeof CASHBACK_MILESTONES[number])) {
